...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Android 13** | API 33 | `common-android13-5.15` | A13-GAPPS, A13-GAPIS, A13-AOSP | `10871489` | Auto default: KernelSU `v3.3.0`. |
| **Android 14** | API 34 | `common-android14-6.1` | A14-GAPPS, A14-GAPIS, A14-AOSP | `9964412` | Auto default: KernelSU `v3.3.0`. |
| **Android 15** | API 35 | `common-android15-6.6` | A15-GAPPS, A15-GAPIS, A15-AOSP (No 16KB Page) | `11987101` | Auto default: KernelSU `v3.3.0` with x86_64 hardening patches baked in. |
| **Android 16** | API 36.0 | `common-android15-6.6-2025-02` | A16-GAPPS, A16-GAPIS, A16-AOSP (No 16KB Page) | `13070261` | API 36.0 AVD still uses android15-6.6-2025-02 GKI. Auto default: KernelSU `v3.3.0`. |
| **Android 16** | API 36.1 | `common-android16-6.12` | A16-GAPPS, A16-GAPIS, A16-AOSP (No 16KB Page) | `13996879` | API 36.1 moves to 6.12 GKI. Auto default: KernelSU `v3.3.0`. |

- KernelSU `v3.2.0` introduced [Bring back x86_64 support with a catch by @hmtheboy154 in \#3328](https://github.com/tiann/KernelSU/pull/3328). The workflow defaults all AVD targets to the latest stable KernelSU release, `v3.3.0`, and applies the appropriate syscall compatibility path per kernel version.
...
| **KSU Ref** | `auto` uses the per-target default `v3.3.0`. You may also provide a tag, branch, or commit ref for testing. |
...
Use **Release AVD GKI Kernels** for publishing. Select the KernelSU project and enter a version tag such as `v3.2.0` or the latest stable `v3.3.0`.
...
For AVD x86_64 testing, `auto` now defaults to **KernelSU `v3.3.0` for all targets**. A13/A14 still use the older syscall table path, while A15+ uses the newer switch-case hardening path that needs the additional kernel patching.
...
The workflow first integrates KernelSU and detects whether the selected ref contains the `X86_FEATURE_INDIRECT_SAFE` checks. The `v3.3.0` defaults use the matching syscall compatibility path for each kernel generation.
